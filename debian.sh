...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/friends122/izin/main/ip | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/friends122/izin/main/ip | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
Exp1=$(curl https://raw.githubusercontent.com/friends122/izin/main/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
restart_system() {
    USRSC=$(wget -qO- https://raw.githubusercontent.com/friends122/izin/main/ip | grep $ipsaya | awk '{print $2}')
    EXPSC=$(wget -qO- https://raw.githubusercontent.com/friends122/izin/main/ip | grep $ipsaya | awk '{print $3}')
...
<code>────────────────────</code>
<i>Automatic Notification from Github Dragon Emperor</i>
